...
export class RecursosComponent implements OnInit {
  ...
  async subirImagen(event: Event) {
    ...
    const { error } = await this.supabaseService.client
      .storage.from('imagenes').upload(path, file);
    if (!error) {
      const { data } = this.supabaseService.client
        .storage.from('imagenes').getPublicUrl(path);
  ...
  async guardarRecurso() {
    ...
    if (this.modoEdicion && this.recursoSeleccionado.id) {
      await this.supabaseService.client
        .from('recursos').update(payload).eq('id', this.recursoSeleccionado.id);
    } else {
      await this.supabaseService.client
        .from('recursos').insert([payload]);
    }
  ...
  async eliminarRecurso() {
    ...
    await this.supabaseService.client
      .from('recursos').delete().eq('id', this.recursoAEliminar.id);
    await this.cargarRecursos();
